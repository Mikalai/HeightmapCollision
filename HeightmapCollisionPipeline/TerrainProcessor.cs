...
namespace HeightmapCollisionPipeline
{
    ...
    public class TerrainProcessor : ContentProcessor<Texture2DContent, ModelContent>
    {
                                             ...
                                             ContentProcessorContext context)
        {
            ...
            var height = new float[heightfield.Width, heightfield.Height];
            for (int y = 0; y < heightfield.Height; y++)
            {
                for (int x = 0; x < heightfield.Width; x++)
                {
                    ...
                    height[x, y] = (heightfield.GetPixel(x, y) - 1) * terrainBumpiness;
                }
            }

            model.Tag = new HeightMapInfo(height, terrainScale);
